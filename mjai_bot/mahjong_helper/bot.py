...
class Bot(AkagiBot):
    ...
    def _think_discard(self) -> str:
        ...
        analysis = self._engine.analyze_discard(hand_tiles, melds=melds)
        ...
        action = {
            ...
            "pai": discard_tile,
            "tsumogiri": discard_tile == self.last_self_tsumo,
        }
    ...
    def _think_call(self) -> str:
        ...
        if not analysis.should_call or not analysis.call_type:
            return self._action({"type": "none"})

        if analysis.call_type == "chi" and self.can_chi:
            consumed = self.find_chi_consume_simple()
            if consumed:
                return self._action({
                    "type": "chi",
                    "actor": self.player_id,
        ...
        if analysis.call_type == "pon" and self.can_pon:
            consumed = self.find_pon_consume_simple()
            if consumed:
                return self._action({
                    "type": "pon",
                    "actor": self.player_id,
        ...
        if analysis.call_type == "daiminkan" and self.can_daiminkan:
    ...
    def _fallback_discard_action(self) -> str:
        hand_tiles = self._current_hand_tiles()
        tile = self.last_self_tsumo if self.last_self_tsumo not in (None, "?") else None
    ...
    def _current_hand_tiles(self, include_tsumo: bool = True) -> list[str]:
        ...
        if include_tsumo and len(tiles) % 3 == 1 and self.last_self_tsumo not in (None, "?"):
            tiles.append(self.last_self_tsumo)
        return tiles
    ...
    def _select_discard_tile(
        ...
        if analysis.best_tile is None:
            if self.last_self_tsumo not in (None, "?"):
                return self.last_self_tsumo
            return hand_tiles[0] if hand_tiles else None
        selected = _select_tile_from_hand(analysis.best_tile, hand_tiles)
        if selected:
            return selected
        if self.last_self_tsumo not in (None, "?"):
            return self.last_self_tsumo
    ...
    def _track_melds(self, events: list[dict]) -> None:
        for event in events:
